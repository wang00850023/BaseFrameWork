...
public class TimeUtil {
	...
	public static String getDescriptionTimeFromTimestamp(long timestamp) {
		...
		if (String.valueOf(timestamp).length() != 13) {
			timestamp = Long.valueOf(timestamp) * 1000;
		}	
	...
	public static String getDescriptionTimeFromTimestamp(String timestamp) {
		if (isNumeric(timestamp)) {
			...
			if (String.valueOf(timestamp).length() != 13) {
				 timestamps = Long.valueOf(timestamp) * 1000;
			}			
			long currentTime = System.currentTimeMillis();
			long timeGap = (currentTime - timestamps) / 1000;// 与现在时间相差秒数
			System.out.println("timeGap: " + timeGap);
			String timeStr = null;
			if (timeGap > YEAR) {
				timeStr = timeGap / YEAR + "年前";
	...
	public static String longToString(String currentTime, String formatType) {
		...
		if (isNumeric(currentTime)) {
			...
			Date date = longToDate(time, formatType);// long类型转成Date类型
			strTime = dateToString(date, formatType); // date类型转成String
		}
		return strTime;
	}
	...
	public static String getChatTime(long timesamp) {
		...
		if (String.valueOf(timesamp).length() != 13) {
